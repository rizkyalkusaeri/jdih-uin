...
<template>
  ...
  <GuestLayout>
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
      ...
      <!-- Glossary List -->
      <div v-if="glossaries.data.length > 0" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        <div v-for="item in glossaries.data" :key="item.id"
      ...
      <!-- Pagination -->
      <div class="mt-10 flex justify-center text-sm" v-if="glossaries.links.length > 3">
        <div class="flex flex-wrap justify-center gap-1.5 bg-white p-2 rounded-xl border border-gray-100 shadow-sm">
          <template v-for="(link, key) in glossaries.links" :key="key">
